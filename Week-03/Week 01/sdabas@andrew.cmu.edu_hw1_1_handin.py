...
# hw1.py
# Name: sachin dabas
# Andrew id: sdabas@andrew.cmu.edu
#################################################
...
def distance(x1, y1, x2, y2):
    distance = math.sqrt(((x2 - x1) ** 2) + ((y2 - y1) ** 2))    # ** is used 
    # for exponential // use math.sqrt for math functions 
    return distance

def circlesIntersect(x1, y1, r1, x2, y2, r2):
    d = distance(x1,x2,y1,y2)
    if d <= (r1 + r2): #if distance is equal to sum of radius , it returns true
        return True
    else: 
        return False 

def getInRange(x, bound1, bound2):
    lo = min(bound1, bound2) #min and max functions
    hi = max(bound1, bound2)
    return min(max(x, lo),hi)

def eggCartons(eggs):
    if(eggs % 12 == 0): return eggs//12 # // is used for integer values
    else:
        return (eggs // 12) + 1 #math.ceil(eggs/12) returns nearest max

def pascalsTriangleValue(row, col):
    if ((row < 0) or (col < 0) or (col > row)): return None 
    return roundHalfUp(
        (math.factorial(row)) / 
        (math.factorial(col) * math.factorial(row - col)) ) #roundhalfup to 
        #nearest integer

def getKthDigit(n, k):
    n = abs(n) #non negative
    return (n// (10 ** k)) % 10

def setKthDigit(n, k, d):
    a = abs(n) - getKthDigit(abs(n),k) * (10 ** k)
    return  a + (d * (10 ** k))
...
def numberOfPoolBalls(rows):
    rows = abs(rows) #non negative integer values
    return roundHalfUp(rows * (rows + 1) / 2)

def numberOfPoolBallRows(balls):
    balls = abs(balls)
    numofRows = (-1 + math.sqrt(1 + 8 * balls)) / 2
    if numofRows - int(numofRows) == 0: return int(numofRows)
    else: 
        return int(numofRows + 1) 

def colorBlender(rgb1, rgb2, midpoints, n):
    if n < 0: return None
    elif n > (midpoints + 1): return None
    else:
        a1 = (rgb1 // 10**6) % 10**3
        a2 = (rgb1 // 10**3) % 10**3
        a3 = rgb1 % 10**3
        ...
        r1 = roundHalfUp(a1 + ((b1 - a1) / (midpoints + 1)) * n)
        r2 = roundHalfUp(a2 + ((b2-a2) / (midpoints + 1)) * n)
        r3 = roundHalfUp(a3 + ((b3-a3) / (midpoints + 1)) * n)

        return r1 * (10 ** 6) + r2 * (10 ** 3)  + r3     
